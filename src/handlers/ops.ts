...
/**
 * The market-wide unsigned aggregates behind GET /v1/stats — shared with the
 * history sampler (stats-history.ts), which persists the same numbers as a
 * durable time-series.
 */
export async function computeMarketAggregates(now: number): Promise<MarketAggregates> {
  ...
  return {
    providers: {
      total: totals.providers as number,
      active: liveProviders,
      busy: busyProviders,
      free: Math.max(0, liveProviders - busyProviders),
    },
    offers: { active: totals.active_offers as number, live: live.length, busy: busyOffers },
    attestations: { valid: totals.attestations as number },
    capacity: { liveCores, liveRamGib },
    price: prices.length === 0 ? null : { min: prices[0]!, median: median!, max: prices[prices.length - 1]! },
  };
}
...
export async function getStats(req: Request, server: Server<unknown>): Promise<Response> {
  ...
  if (!blob) {
    const now = Date.now();
    const agg = await computeMarketAggregates(now);
