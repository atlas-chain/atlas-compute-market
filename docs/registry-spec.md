...
- **Segment 0 and the final leaf are always opened**, in addition to the `K` sampled segments: segment 0 anchors the chain at `s_0` (else a chain not bound to the seed/nonce/provider passes whenever segment 0 escapes the sample), and a Merkle path for the last checkpoint binds the submitted `s_L` to the committed root.
